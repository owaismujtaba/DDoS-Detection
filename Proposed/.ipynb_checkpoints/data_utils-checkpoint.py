...
def load_dataset(PATH, nrows):
    ...
    filenames = ['TFTP.csv', 'DrDoS_SNMP.csv',  'DrDoS_DNS.csv', 'DrDoS_MSSQL.csv','DrDoS_SSDP.csv','DrDoS_NetBIOS.csv','DrDoS_LDAP.csv', 'DrDoS_NTP.csv', 'Syn.csv', 'UDPLag.csv',  'DrDoS_UDP.csv']
...
def create_pca_dataset(components=41): 
    ...
    dataset[' Label'] = encoder.fit_transform(xy[' Label'])
    ...
    train_PCA, test_PCA, train_label, test_label = train_test_split(principal_components, dataset[' Label'], test_size=0.33, random_state=42)
